...
class Args:
    ...
    wandb_entity: str = "michael4hua-technical-university-of-munich"
...
def make_env(config, args, gamma):
    ...
    env = gym.wrappers.vector.RecordEpisodeStatistics(env) # for wandb log
    env = gym.wrappers.vector.NormalizeReward(env, gamma=gamma) # might help
...
class Agent(nn.Module):
    ...
    def get_action_and_value(self, x, action=None):
        ...
        log_std = self.actor_logstd(base)
    ...
    def forward(self, x):
        """Pure inference: returns deterministic action (mean)"""
        base = self.actor_base(x)
        mean = self.actor_mean(base)
        return mean
